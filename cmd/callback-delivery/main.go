...
func main() {
	...
	telemetryShutdown, err := service.InitTelemetry(ctx, cfg.Telemetry, "callback-delivery", version.Version, metrics.Registry, logger)
	if err != nil {
	...
	deliverySvc := callback.NewDeliveryService(cfg, registry.CallbackDedup, registry.Stump, registry.CallbackURLRegistry, logger)
